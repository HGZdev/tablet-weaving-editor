import type { Draft, MakeDirChange, DirChangeTurn, Skew } from "./types";
...
export const genThread = ({
  ...
  // Use the actual length of dirsChanges for the column to avoid accessing out-of-bounds
  const actualPicks = props.dirsChanges[props.col]?.length || picks;
  return [...new Array(actualPicks)].map((_x, pick) =>
    genSqrt({ row: pick, ...props })
  );
};
